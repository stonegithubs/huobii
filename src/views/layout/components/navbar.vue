    ...
    >
      <slot name="nav-item">
        <router-link :to="{ name: 'trade', params:{ option: 'buy', coin: 'btc' } }">
          <el-menu-item index="1">{{ $t("navbar.trade") }}</el-menu-item>
        </router-link>
      ...
      <div class="right-nav">
        <el-submenu v-if="checkAuth" index="4">
          <template slot="title">
            <i class="iconfont icon-file-text"/>
            {{ $t("navbar.user.order") }}
          </template>

          <router-link :to="{ name: 'order'}">
            <el-menu-item index="4-1">{{ $t("navbar.user.exchangeOrder") }}</el-menu-item>
          </router-link>
          <router-link :to="{ name: 'advList'}">
            <el-menu-item index="4-2">{{ $t("navbar.user.financeOrder") }}</el-menu-item>
          </router-link>
        </el-submenu>

        <router-link v-if="checkAuth" :to="{ name: 'wallet'}">
          <i class="iconfont icon-wallet"/>
          {{ $t("navbar.user.finance") }}
        </router-link>
        <el-submenu v-if="checkAuth" index="5">
          <template slot="title">
            <i class="iconfont icon-user"/>
            {{ $t("navbar.user.userProfile") }}
          </template>
