      ...
      >
        ...
        <template slot="suffix">
          <el-button
            ...
            class="send-code"
            @click="msendCode"
          >{{ timeRest===60? '':timeRest }}{{ buttonInner }}</el-button>
        </template>
      </el-input>
    </el-form-item>
    <el-form-item
      :label="$t('captcha')"
      :rules="[{ required: true, message: $t('login.captchaIsRequired'), trigger: 'blur' }]"
      prop="code"
    >
      <el-input
...
export default {
  ...
  methods: {
    ...
    msendCode() {
      ...
      // console.log('已经发送')
      const regionPhone =
        this.getCountryCodeByAbbr(this.loginForm.region) + this.loginForm.phone;
      sendCaptcha1(regionPhone, this.loginForm.region).then(responese => {
        setTimeout(() => {
          getCaptcha(regionPhone, this.loginForm.region)
            .then(responese1 => {
