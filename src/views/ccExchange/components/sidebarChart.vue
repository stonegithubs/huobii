<template>
  <div class="sidebar-chart">
    <div v-if="this.$store.state.user.token" class="total-balance">
      <div v-loading="balanceInfoLoading" element-loading-background="#1b1e2e">
    ...
    <div v-else class="not-login">
      <p>
        <router-link :to="{ name: 'login'}">登录</router-link> 或 <router-link :to="{ name: 'registry'}">注册</router-link>开始交易
      </p>
    </div>
    <div class="drawer">
      <el-tabs class="trade-bar" v-model="activeName" type="border-card" @tab-click="handleClick">
        <el-tab-pane v-for="symbol in symbols" :key="symbol.symbolName" :name="symbol.symbolName">
          ...
          <el-table class="mini-trade" height="600" border :data="symbolList" :default-sort="{prop: 'close', order: 'descending'}" max-height="800" @row-click="handleRowClick" v-loading="symbolList.length == 0" element-loading-background="#181b2a" style="width: 100%">
            <el-table-column label="币种" sortable min-width="70">
              <template slot-scope="scope"><span class="upper">{{ scope.row.symbolName }}</span></template>
            </el-table-column>
            <el-table-column prop="close" label="最新价" sortable :formatter="priceFormatter" min-width="80">
            </el-table-column>
            <el-table-column prop="amount" label="成交量" sortable :formatter="amountFormatter" min-width="80">
            </el-table-column>
          </el-table>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  created() {
    ...
    if (this.$store.state.coinData.symbolList.length === 0) {
      this.$store.dispatch('getSymbolList').then(response => {
      }).catch(error => {
