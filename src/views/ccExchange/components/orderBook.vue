<template>
  <div class="order-book">
    <div class="header hb-tab">
      <div>
        {{ $t('index.tradeShow.lastPrice') }}
        <span
          v-loading="latestLoading"
        >{{ getLatestPrice }} {{ this.$store.state.coinData.mainCoin.toUpperCase() }}</span>
    ...
    <div class="inner" v-loading="this.buyDepthList.length == 0">
      <div class="inner-wrapper">
        ...
        >
          ...
          <el-table-column
            :label="$t('exchange.main.total')+this.$store.state.coinData.targetCoin.toUpperCase()"
          >
            <template slot-scope="scope">{{ scope.row.accumulative.toFixed(6) }}</template>
          </el-table-column>
        </el-table>
...
<script>
import { getMarketDetail, getMarketDepth } from "../../../api/coins";
...
export default {
  ...
  watch: {
    targetCoin: function() {
      const symbol =
        this.$store.state.coinData.targetCoin +
        this.$store.state.coinData.mainCoin;
