<template>
    <el-container class="trade">
      ...
      <el-form background-color='#f2f3f8' :model="verifyForm" :rules="verifyRule" ref="verifyForm">
    ...
    <el-dialog :modal="true" title="高级认证" :visible.sync="advancedVerifyVisible" :lock-scroll="true" center :modal-append-to-body="false">
      <div style="text-align: center;">
        <div class="dialog-info">
          ...
          <el-upload class="upload-demo" ref="upload" action="/api/v1/user/verify_advanced" list-type="picture" :limit="3" :on-success="advanceNext" :on-error="handleAdvanceError" :before-upload="beforeAvatarUpload" :on-exceed="handleExceed" :file-list="fileList"
            :headers="{'token': this.$store.state.user.token}" :auto-upload="false">
            <!--todo:token名字在这里写死了-->
