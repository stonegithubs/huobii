<template>
  <div class="trade-select">
    <div class="trade-menu hidden-xs-only">
      <div class="menu-list">
        <p>购买</p>
        <a v-for="item in supportedCoin" href="javascript:;" @click="handleCoinChoose(item.id, 'buy')">{{ item.name }}</a>
      </div>
      <div class="menu-list">
        <p>出售</p>
        <a v-for="item in supportedCoin" href="javascript:;" @click="handleCoinChoose(item.id, 'sell')">{{ item.name }}</a>
      </div>
    </div>
...
export default {
  ...
  methods: {
    handleCoinChoose(id, type) {
      if (type === 'sell' || type === '出售') {
        console.log('麦', id)
      }
      if (type === 'buy' || type === '购买') {
        console.log('买', id)
